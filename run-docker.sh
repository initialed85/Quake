...
if [[ "${1}" == "" ]] || [[ "${1}" == "up" ]]; then
    if [[ "${DEBUG}" == "1" ]]; then
        docker run --stop-timeout=0 --platform="linux/${desired_arch}" --privileged -d --net=host -v "$(pwd)/id1:/srv/WinQuake/id1" --name quake --entrypoint lldb-server initialed85/quake:latest platform --listen "*:11111" --server
    else
