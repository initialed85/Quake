...
#define MAX_VESA_MODES                                                         \
  30 // we'll just take the first 30 if there
     //  are more
typedef struct {
...
void VID_ExtraSwapBuffers(viddef_t *vid, vmode_t *pcurrentmode, vrect_t *rects);
...
void VID_InitExtra(void) {
  ...
  *(long *)pinfoblock->VbeSignature =
  ...
  pmodenums =
      (short *)VID_ExtraFarToLinear(*(byte **)&pinfoblock->VideoModePtr[0]);
  ...
  while ((*pmodenums != -1) && (nummodes < MAX_VESA_MODES)) {
    if (VID_ExtraGetModeInfo(*pmodenums)) {
      ...
      if (modeinfo.mode_attributes & LINEAR_FRAME_BUFFER) {
        ...
        vesa_extra[nummodes].plinearmem = real2ptr(phys_mem_info.address);
      } else {
        // banked at 0xA0000
        vesa_extra[nummodes].vesamode = modeinfo.modenum;
        vesa_extra[nummodes].pages[0] = 0;
        vesa_extra[nummodes].plinearmem = real2ptr(modeinfo.winasegment << 4);
...
int VID_ExtraInitMode(viddef_t *lvid, vmode_t *pcurrentmode) {
  ...
  if (!pextra->vga_incompatible && (lvid->numpages == 3) && de_exists &&
  ...
  if (lvid->numpages > 1) {
    ...
    while (rects) {
      VGA_UpdateLinearScreen(
          ...
          VGA_pagebase + rects->x + (rects->y * VGA_rowbytes), rects->width,
          rects->height, lvid->rowbytes, VGA_rowbytes);
