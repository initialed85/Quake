...
*/
// vid_sunxil.c -- uses X to setup windows and XIL to copy images (scaled as
// needed) 		   to screen
...
#define STD_EVENT_MASK                                                         \
  (StructureNotifyMask | KeyPressMask | KeyReleaseMask | ButtonPressMask |     \
   ButtonReleaseMask | ExposureMask | PointerMotionMask | FocusChangeMask)
...
#define PM(a)                                                                  \
  (int)((current_pixel_multiply) ? ((a) * current_pixel_multiply) : (a))
#define MP(a)                                                                  \
  (int)((current_pixel_multiply) ? ((a) / current_pixel_multiply) : (a))
...
void *update_thread() {
  ...
  while (read(render_pipeline[0], &image, sizeof(image)) == sizeof(image)) {
    ...
    if (current_pixel_multiply < 2)
      ...
      xil_scale(image, display_image, "nearest", (float)current_pixel_multiply,
