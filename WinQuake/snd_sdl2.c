...
qboolean SNDDMA_Init(void)
{
	...
	if (desired_bits == 16)
		desired.format = SDL_BYTEORDER == SDL_LIL_ENDIAN ? AUDIO_S16LSB : AUDIO_S16MSB;
	else if (desired_bits == 32)
		desired.format = SDL_BYTEORDER == SDL_LIL_ENDIAN ? AUDIO_F32LSB : AUDIO_F32MSB;
	...
	// {
	// 	SDL_Log("Failed because obtained.freq: %d != desired.freq: %d", obtained.freq, desired.freq);
	// 	return false;
	// }
	...
	// {
	// 	SDL_Log("Failed because obtained.format: %d != desired.format: %d", obtained.format, desired.format);
	// 	return false;
	// }
	...
	// {
	// 	SDL_Log("Failed because obtained.channels: %d != desired.channels: %d", obtained.channels, desired.channels);
	// 	return false;
	// }
	...
	// {
	// 	SDL_Log("Failed because obtained.samples: %d != desired.samples: %d", obtained.samples, desired.samples);
	// 	return false;
	// }
	...
	shm->splitbuffer = 0;
	shm->samplebits = (desired.format & 0xFF); // the first 8-bits is the sample bit size
