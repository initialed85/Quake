...
int IPX_Write(int handle, byte *buf, int len, struct qsockaddr *addr) {
  ...
  case 0xff: // unable to send packet
    ...
    if (IPX_GetLocalTarget(&lma->socketbuffer[handle][0].header.destination,
                           lma->socketbuffer[handle][0].ecb.immediateAddress) !=
        0)
      return -1;
...
int IPX_AddrCompare(struct qsockaddr *addr1, struct qsockaddr *addr2) {
  ...
  if (Q_memcmp(&((struct sockaddr_ipx *)addr1)->sipx_addr,
