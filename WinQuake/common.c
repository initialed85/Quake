...
int Q_strcasecmp(char *s1, char *s2) { return Q_strncasecmp(s1, s2, 99999); }
...
byte *COM_LoadHunkFile(char *path) { return COM_LoadFile(path, 1); }

byte *COM_LoadTempFile(char *path) { return COM_LoadFile(path, 2); }
...
void COM_AddGameDirectory(char *dir) {
  ...
  //
  search = Hunk_AllocName(sizeof(searchpath_t), dir);
  ...
  for (i = 0;; i++) {
    ...
    if (!pak)
      break;
    search = Hunk_AllocName(sizeof(searchpath_t), pakfile);
    search->pack = pak;
    search->next = com_searchpaths;
    com_searchpaths = search;
  }
...
void COM_InitFilesystem(void) {
  ...
  if (i) {
    ...
    while (++i < com_argc) {
      ...
      search = Hunk_AllocName(sizeof(searchpath_t), com_argv[i]);
