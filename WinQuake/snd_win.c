...
HRESULT(WINAPI *pDirectSoundCreate)(GUID FAR *lpGUID, LPDIRECTSOUND FAR *lplpDS,
...
sndinitstat SNDDMA_InitDirect(void) {
  ...
  while ((hresult = iDirectSoundCreate(NULL, &pDS, NULL)) != DS_OK) {
    ...
    if (MessageBox(NULL,
                   ...
                   "Select Retry to try to start sound again or Cancel to run "
                   "Quake with no sound.",
                   "Sound not available",
  ...
  while ((hresult = pDSBuf->lpVtbl->Lock(pDSBuf, 0, gSndBufSize, &lpData,
...
qboolean SNDDMA_InitWav(void) {
  ...
  while ((hr = waveOutOpen((LPHWAVEOUT)&hWaveOut, WAVE_MAPPER, &format, 0, 0L,
