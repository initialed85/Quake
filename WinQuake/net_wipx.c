...
int WIPX_OpenSocket(int port) {
  ...
  if (psetsockopt(newsocket, SOL_SOCKET, SO_BROADCAST, (char *)&_true,
                  sizeof(_true)) < 0)
    goto ErrorReturn;
...
int WIPX_Connect(int handle, struct qsockaddr *addr) { return 0; }
...
int WIPX_Read(int handle, byte *buf, int len, struct qsockaddr *addr) {
  ...
  ret = precvfrom(socket, packetBuffer, len + 4, 0, (struct sockaddr *)addr,
                  &addrlen);
  if (ret == -1) {
...
int WIPX_Write(int handle, byte *buf, int len, struct qsockaddr *addr) {
  ...
  *(int *)(&packetBuffer[0]) = sequence[handle];
  ...
  ret = psendto(socket, packetBuffer, len, 0, (struct sockaddr *)addr,
