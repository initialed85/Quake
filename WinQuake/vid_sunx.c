...
void ResetFrameBuffer(void) {
  ...
  //		sizeof(*d_pzbuffer));
  d_pzbuffer = (short *)Hunk_HighAllocName(
      vid.width * vid.height * sizeof(*d_pzbuffer), "zbuff");

  x_framebuffer[0] = XCreateImage(x_disp, x_vis, x_visinfo->depth, ZPixmap, 0,
                                  Z_Malloc(mem), vid.width, vid.height, 32, 0);
...
void ResetSharedFrameBuffers(void) {
  ...
  //		Z_Free(d_pzbuffer);
  d_pzbuffer =
      Hunk_HighAllocName(vid.width * vid.height * sizeof(*d_pzbuffer), "zbuff");
  ...
  for (frm = 0; frm < 2; frm++) {
    ...
    x_framebuffer[frm] =
        XShmCreateImage(x_disp, x_vis, x_visinfo->depth, ZPixmap, 0,
                        &x_shminfo[frm], vid.width, vid.height);
...
void VID_Init(unsigned char *palette) {
  ...
  // setup attributes for main window
  {
    ...
    tmpcmap = XCreateColormap(x_disp, XRootWindow(x_disp, x_visinfo->screen),
                              x_vis, AllocNone);
...
void GetEvent(void) {
  ...
  if (mouse_avail) {
    if (key_dest == key_game && !mouse_grabbed && mouse_in_window) {
      mouse_grabbed = true;
      /* grab the pointer */
      XGrabPointer(x_disp, x_win, True, 0, GrabModeAsync, GrabModeAsync, x_win,
                   None, CurrentTime);
      // inviso cursor
  ...
  if (doShm) {
    ...
    while (rects) {
      ...
      if (x_visinfo->depth == 16)
        st2_fixup(x_framebuffer[current_framebuffer], rects->x, rects->y,
                  rects->width, rects->height);
      else if (x_visinfo->depth == 24)
        st3_fixup(x_framebuffer[current_framebuffer], rects->x, rects->y,
                  rects->width, rects->height);
      if (!XShmPutImage(x_disp, x_win, x_gc, x_framebuffer[current_framebuffer],
    ...
    while (rects) {
      if (x_visinfo->depth == 16)
        ...
        st3_fixup(x_framebuffer[current_framebuffer], rects->x, rects->y,
                  rects->width, rects->height);
      XPutImage(x_disp, x_win, x_gc, x_framebuffer[0], rects->x, rects->y,
...
int I_OpenWindow(void) { return 0; }
