...
struct hostent FAR *(PASCAL FAR *pgethostbyaddr)(const char FAR *addr, int len,
                                                 int type);
...
void WINS_GetLocalAddress() {
  ...
  addr = ntohl(myAddr);
  sprintf(my_tcpip_address, "%d.%d.%d.%d", (addr >> 24) & 0xff,
...
int WINS_Init(void) {
  ...
  // initialize the Winsock function vectors (we do this instead of statically
  // linking so we can run on Win 3.1, where there isn't necessarily Winsock)
  ...
  if (!pWSAStartup || !pWSACleanup || !pWSAGetLastError || !psocket ||
      !pioctlsocket || !psetsockopt || !precvfrom || !psendto ||
      !pclosesocket || !pgethostname || !pgethostbyname || !pgethostbyaddr ||
      !pgetsockname) {
...
int WINS_Write(int socket, byte *buf, int len, struct qsockaddr *addr) {
  ...
  ret = psendto(socket, buf, len, 0, (struct sockaddr *)addr,
...
int WINS_GetNameFromAddr(struct qsockaddr *addr, char *name) {
  ...
  hostentry = pgethostbyaddr((char *)&((struct sockaddr_in *)addr)->sin_addr,
