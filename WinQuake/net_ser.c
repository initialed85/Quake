  ...
  while (1) {
    ...
    case STATE_LENGTH2:
      ...
      if (p->mtype == MTYPE_RELIABLE && p->lengthStated > MAX_MSGLEN) {
        ...
        if (p->mtype < MTYPE_ACK) {
          *(short *)&p->sock
               ->receiveMessage[p->sock->receiveMessageLength + 1] =
              p->lengthStated;
          p->lengthFound += 2;
        }
      }
    ...
    case STATE_DATA:
      p->sock->receiveMessage[p->sock->receiveMessageLength + p->lengthFound] =
          b;
      p->lengthFound++;
      CRC_ProcessByte(&p->crcValue, b);
...
void Serial_Listen(qboolean state) { listening = state; }

qboolean Serial_CanSendMessage(qsocket_t *sock) { return sock->canSend; }
