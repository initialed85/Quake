...
void PR_StackTrace(void) {
  ...
  for (i = pr_depth; i >= 0; i--) {
    ...
    if (!f) {
      ...
      Con_Printf("%12s : %s\n", PR_StrQCToC(f->s_file), PR_StrQCToC(f->s_name));
  }
}
...
void PR_Profile_f(void) {
  ...
  do {
    ...
    if (best) {
      if (num < 10)
        Con_Printf("%7i %s\n", best->profile, PR_StrQCToC(best->s_name));
      num++;
      best->profile = 0;
    }
  } while (best);
...
void PR_ExecuteProgram(func_t fnum) {
  ...
  while (1) {
    ...
    case OP_NOT_V:
      ...
      break;
    case OP_NOT_S:
      c->_float = !a->string || !*PR_StrQCToC(a->string);
      break;
    case OP_NOT_FNC:
      c->_float = !a->function;
      break;
    ...
    case OP_EQ_V:
      ...
      break;
    case OP_EQ_S:
      c->_float = !strcmp(PR_StrQCToC(a->string), PR_StrQCToC(b->string));
      break;
    case OP_EQ_E:
