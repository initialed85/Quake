...
qboolean VID_SetWindowedMode(int modenum) {
  ...
  dibwindow = CreateWindowEx(ExWindowStyle, "WinQuake", "GLQuake", WindowStyle,
...
int VID_SetMode(int modenum, unsigned char *palette) {
  ...
  if ((windowed && (modenum != 0)) || (!windowed && (modenum < 1)) ||
...
void CheckTextureExtensions(void) {
  ...
  while (*tmp) {
    if (strncmp((const char *)tmp, TEXTURE_EXT_STRING,
  ...
  if ((bindTexFunc = (BINDTEXFUNCPTR)wglGetProcAddress(
           (LPCSTR) "glBindTextureEXT")) == NULL) {
...
void CheckArrayExtensions(void) {
  ...
  tmp = (unsigned char *)glGetString(GL_EXTENSIONS);
  while (*tmp) {
    if (strncmp((const char *)tmp, "GL_EXT_vertex_array",
...
#else
void CheckMultiTextureExtensions(void) { gl_mtexable = true; }
#endif
...
void VID_SetDefaultMode(void) { IN_DeactivateMouse(); }
...
byte shiftscantokey[128] = {
    ...
    K_F4,        K_F5, // 3
    K_F6,        K_F7,      K_F8,   K_F9,    K_F10,       K_PAUSE, 0,
    K_HOME,      K_UPARROW, K_PGUP, '_',     K_LEFTARROW, '%',     K_RIGHTARROW,
    '+',         K_END, // 4
    K_DOWNARROW, K_PGDN,    K_INS,  K_DEL,   0,           0,       0,
    K_F11,       K_F12,     0,      0,       0,           0,       0,
    0,           0, // 5
...
void VID_InitFullDIB(HINSTANCE hInstance) {
  ...
  do {
    ...
    if ((devmode.dmBitsPerPel >= 15) && (devmode.dmPelsWidth <= MAXWIDTH) &&
      ...
      if (ChangeDisplaySettings(&devmode, CDS_TEST | CDS_FULLSCREEN) ==
        ...
        sprintf(modelist[nummodes].modedesc, "%dx%dx%d", devmode.dmPelsWidth,
...
qboolean VID_Is8bit() { return is8bit; }
...
void VID_Init(unsigned char *palette) {
          ...
          if (COM_CheckParm("-height")) {
            ...
            for (i = 1, vid_default = 0; i < nummodes; i++) {
              if ((modelist[i].width == width) &&
                  (modelist[i].height == height) && (modelist[i].bpp == bpp)) {
                vid_default = i;
                done = 1;
                break;
              }
  ...
  if (!baseRC)
    Sys_Error("Could not initialize GL (wglCreateContext failed).\n\nMake sure "
              "you in are 65535 color mode, and try running -window.");
  if (!wglMakeCurrent(maindc, baseRC))
    Sys_Error("wglMakeCurrent failed");
