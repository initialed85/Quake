...
static void CheckMouseState(void) {
  if (x_focus && _windowed_mouse.value && !x_grabbed) {
    ...
    if (XGetPointerControl(x_disp, &x_mouse_num, &x_mouse_denom,
                           &x_mouse_thresh))
    ...
    //		if (XChangePointerControl( x_disp, True, True, 1, MOUSE_SCALE,
    ...
    x_grabbed = false;
    // undo mouse warp
    if (XChangePointerControl(x_disp, True, True, x_mouse_num, x_mouse_denom,
                              x_mouse_thresh))
