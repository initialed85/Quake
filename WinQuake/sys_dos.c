...
void *dos_getmaxlockedmem(int *size) {
  ...
  if (!win95) /* Not windows or earlier than Win95 */
  {
    ...
    working_size =
        meminfo.largest_available_free_block_in_bytes - LEAVE_FOR_CACHE;
  }
  ...
  if (!win95) {
    ...
    if (j) {
      allocsize =
          ((int)(Q_atoi(com_argv[j + 1]))) * 0x100000 + LOCKED_FOR_MALLOC;
