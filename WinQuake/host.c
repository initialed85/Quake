...
cvar_t sys_ticrate = {"sys_ticrate", "0.02"};
...
void Host_Init(quakeparms_t *parms) {
  ...
  if (cls.state != ca_dedicated) {
    ...
    R_Init();
#ifndef _WIN32
    // on Win32, sound initialization has to come before video initialization,
    // so we can put up a popup if the sound hardware is in use
    S_Init();
#else
