/*
Copyright (C) 2025 Edward Beech (initialed85).
...
void VID_SetPalette(unsigned char *palette)
{
	...
	if (SDL_SetPaletteColors(quake_surface->format->palette, sdl_palette, 0, 256) < 0)
	{
		SDL_Log("Failed to SDL_SetPaletteColors(quake_surface->format->palette, sdl_palette, 0, 256): %s", SDL_GetError());
#ifdef __EMSCRIPTEN__
...
{
	// if you don't implement this gamma doesn't work- not sure what other impact it has
...
void VID_Init(unsigned char *palette)
{
	...
	if (!COM_CheckParm("-nosound"))
	{
		...
		{
			SDL_Log("Failed to SDL_Init(SDL_INIT_VIDEO|SDL_INIT_AUDIO): %s", SDL_GetError());
#ifdef __EMSCRIPTEN__
			emscripten_force_exit(1);
#else
			exit(1);
	...
	window = SDL_CreateWindow("Quake", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, screen_width, screen_height, window_flags);
	if (!window)
	{
		SDL_Log("Failed to SDL_CreateWindow(\"Quake\", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, screen_width, screen_height, window_flags): %s", SDL_GetError());
#ifdef __EMSCRIPTEN__
		emscripten_force_exit(1);
#else
		exit(1);
	...
	if (!renderer)
	{
		SDL_Log("Failed to SDL_CreateRenderer(window, -1, SDL_RENDERER_PRESENTVSYNC): %s", SDL_GetError());
#ifdef __EMSCRIPTEN__
		emscripten_force_exit(1);
#else
		exit(1);
	...
	quake_surface = SDL_CreateRGBSurfaceWithFormat(0, screen_width, screen_height, 8, SDL_PIXELFORMAT_INDEX8);
	if (!quake_surface)
	{
		SDL_Log("Failed to SDL_CreateRGBSurfaceWithFormat(0, screen_width, screen_height, 8, SDL_PIXELFORMAT_INDEX8): %s", SDL_GetError());
#ifdef __EMSCRIPTEN__
		emscripten_force_exit(1);
#else
		exit(1);
	...
	if (!texture)
	{
		SDL_Log("Failed to SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STREAMING, screen_width, screen_height): %s", SDL_GetError());
#ifdef __EMSCRIPTEN__
		emscripten_force_exit(1);
#else
		exit(1);
...
void VID_Update(vrect_t *rects)
{
	...
	{
		SDL_Log("Failed SDL_LockTexture(texture, NULL, &pixels, &pitch): %s", SDL_GetError());
#ifdef __EMSCRIPTEN__
		emscripten_force_exit(1);
#else
		exit(1);
	...
	// suck the quake surface pixels out of the var, via the 8-bit colour to 24-bit
	// colour convesion table and onto casted version of the pixels var that's
	...
	{
		SDL_Log("Failed SDL_RenderCopy(renderer, texture, NULL, NULL): %s", SDL_GetError());
#ifdef __EMSCRIPTEN__
		emscripten_force_exit(1);
#else
		exit(1);
