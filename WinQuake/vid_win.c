...
void VID_InitFullDIB(HINSTANCE hInstance) {
  ...
  do {
    ...
    if ((devmode.dmBitsPerPel == 8) && (devmode.dmPelsWidth <= MAXWIDTH) &&
      ...
      if (ChangeDisplaySettings(&devmode, CDS_TEST | CDS_FULLSCREEN) ==
        ...
        sprintf(modelist[nummodes].modedesc, "%dx%d", devmode.dmPelsWidth,
                devmode.dmPelsHeight);

        // if the width is more than twice the height, reduce it by half because
        // this is probably a dual-screen monitor
        if (!COM_CheckParm("-noadjustaspect")) {
          if (modelist[nummodes].width > (modelist[nummodes].height << 1)) {
            modelist[nummodes].width >>= 1;
            modelist[nummodes].halfscreen = 1;
            sprintf(modelist[nummodes].modedesc, "%dx%d",
                    modelist[nummodes].width, modelist[nummodes].height);
          }
        }
    ...
    do {
      ...
      if ((((devmode.dmPelsWidth <= MAXWIDTH) &&
        ...
        if (ChangeDisplaySettings(&devmode, CDS_TEST | CDS_FULLSCREEN) ==
          ...
          // because this is probably a dual-screen monitor
          if (!COM_CheckParm("-noadjustaspect")) {
  ...
  if (!is_mode0x13) {
    ...
    if (cstretch > 0) {
      ...
      sprintf(modelist[istretch].modedesc, "%dx%d", modelist[istretch].width,
...
=================
*/
int VID_NumModes(void) { return nummodes; }
...
qboolean VID_SetWindowedMode(int modenum) {
  ...
  if (!vid_mode_set) {
    mainwindow = CreateWindowEx(
        ExWindowStyle, "WinQuake", "WinQuake", WindowStyle, 0, 0,
  ...
  if (!SetWindowPos(mainwindow, NULL, 0, 0, WindowRect.right - WindowRect.left,
  ...
  if (DDActive) {
    if (mgldc) {
      if (memdc) {
        while (rects) {
          if (vid_stretched) {
            MGL_stretchBltCoord(mgldc, memdc, rects->x, rects->y,
                                rects->x + rects->width,
                                rects->y + rects->height, rects->x << 1,
                                rects->y << 1, (rects->x + rects->width) << 1,
                                (rects->y + rects->height) << 1);
          } else {
            MGL_bitBltCoord(mgldc, memdc, rects->x, rects->y,
                            (rects->x + rects->width),
                            (rects->y + rects->height), rects->x, rects->y,
                            MGL_REPLACE_MODE);
          }
    ...
    if (windc && dibdc) {
      ...
      while (rects) {
        if (vid_stretched) {
          MGL_stretchBltCoord(
              windc, dibdc, rects->x, rects->y, rects->x + rects->width,
              rects->y + rects->height, rects->x << 1, rects->y << 1,
  ...
  if (firstupdate) {
    if (modestate == MS_WINDOWED) {
      ...
      if ((trect.left != (int)vid_window_x.value) ||
        ...
        VID_CheckWindowXY();
        SetWindowPos(
            mainwindow, NULL, (int)vid_window_x.value, (int)vid_window_y.value,
            0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_SHOWWINDOW | SWP_DRAWFRAME);
      }
    }
...
byte scantokey[128] = {
    ...
    K_F4,        K_F5, // 3
    K_F6,        K_F7,      K_F8,   K_F9,    K_F10,       K_PAUSE, 0,
    K_HOME,      K_UPARROW, K_PGUP, '-',     K_LEFTARROW, '5',     K_RIGHTARROW,
    '+',         K_END, // 4
...
LONG WINAPI MainWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_CLOSE:
    ...
    // crash on Win95)
    if (!in_mode_set) {
      if (MessageBox(mainwindow, "Are you sure you want to quit?",
                     "Confirm Exit",
