...
void Host_Name_f(void) {
  ...
  host_client->edict->v.netname = PR_StrCToQC(host_client->name);
...
void Host_Pause_f(void) {
  ...
  if (!pausable.value)
    ...
    if (sv.paused) {
      SV_BroadcastPrintf("%s paused the game\n", PR_StrQCToC(sv_player->v.netname));
    } else {
...
void Host_Spawn_f(void) {
  ...
  if (sv.loadgame) { // loaded games are fully inited allready
    ...
    ent->v.netname = PR_StrCToQC(host_client->name);
...
edict_t *FindViewthing(void) {
  ...
  for (i = 0; i < sv.num_edicts; i++) {
    e = EDICT_NUM(i);
    if (!strcmp(PR_StrQCToC(e->v.classname), "viewthing"))
      return e;
  }
