...
void R_DrawSequentialPoly(msurface_t *s) {
  ...
  if (gl_mtexable) {
    ...
    for (i = 0; i < p->numverts; i++, v += VERTEXSIZE) {
      ...
      nv[0] =
          v[0] + 8 * sin(v[1] * 0.05 + realtime) * sin(v[2] * 0.05 + realtime);
      nv[1] =
          v[1] + 8 * sin(v[0] * 0.05 + realtime) * sin(v[2] * 0.05 + realtime);
      nv[2] = v[2];
...
void R_BlendLightmaps(void) {
  ...
  for (i = 0; i < MAX_LIGHTMAPS; i++) {
    ...
    if (lightmap_modified[i]) {
      ...
      //			gl_lightmap_format, GL_UNSIGNED_BYTE,
      //lightmaps+i*BLOCK_WIDTH*BLOCK_HEIGHT*lightmap_bytes); 			glTexImage2D
      //(GL_TEXTURE_2D, 0, lightmap_bytes 				, BLOCK_WIDTH, theRect->h, 0,
      //				gl_lightmap_format, GL_UNSIGNED_BYTE,
