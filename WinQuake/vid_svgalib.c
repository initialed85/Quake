...
void VID_DescribeMode_f(void) {
  ...
  Con_Printf("%d: %d x %d - ", modenum, modes[modenum].width,
             modes[modenum].height);
...
void VID_NumModes_f(void) { Con_Printf("%d modes\n", VID_NumModes()); }
...
char *VID_ModeInfo(int modenum) {
  ...
  if (modenum == 0) {
    sprintf(modestr, "%d x %d, %d bpp", vid.width, vid.height,
            modes[current_mode].bytesperpixel * 8);
    return (modestr);
  } else {
    return (badmodestr);
